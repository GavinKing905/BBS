...
import com.bbs.cookie.parser.CookieNameConfig;
import com.bbs.cookie.parser.CookieNameHelper;
import com.bbs.cookie.parser.CookieNamePolicyParser;
import com.bbs.cookie.parser.CookieParser;
import com.bbs.cookie.parser.CookieUtils;

/**
 * 类DefaultCookieManager.java的实现描述：TODO 类实现描述
 * 
 * @author jie.xu 2014年11月23日 下午7:15:01
 */
public class DefaultCookieManager implements CookieManager {

	private static final Logger logger = LoggerFactory
			.getLogger(DefaultCookieManager.class);
	/**
	...
	public DefaultCookieManager(HttpServletRequest request,
	...
	protected void init() {
		// 取得当前所有的Cookie值
		Map<CookieNameEnum, CookieNameHelper> newValues = CookieParser
				.loadCookie(request);
		for (CookieNameEnum cookieNameEnum : newValues.keySet()) {
			currentCookieValue.put(cookieNameEnum,
					newValues.get(cookieNameEnum));
		}
	}
	...
	public String get(CookieKeyEnum cookieKeyEnum) {
		...
		CookieNameEnum cookienameEnum = CookieNamePolicyParser
				.getCookieName(cookieKeyEnum);
		...
		if (cookieNameHelper == null) {
			...
			return CookieUtils.filterNullValue(cookieNameHelper
					.getValue(cookieKeyEnum));
		}
	}

	private void assertNotSimple(CookieNameEnum cookienameEnum) {
		CookieNameConfig cookieNameConfig = CookieNamePolicyParser
				.getCookieNamePolicyMap().get(cookienameEnum);
	...
	public String get(CookieNameEnum cookieNameEnum) {
		...
		CookieNameHelper cookieNameHelper = currentCookieValue
				.get(cookieNameEnum);
		if (cookieNameHelper == null) {
			return null;
		} else {
	...
	public void set(CookieKeyEnum cookieKey, String value) {
		...
		// 首先找到该CookieKey所在的CookieName
		CookieNameEnum cookieName = CookieNamePolicyParser
				.getCookieName(cookieKey);
	 ...
	 * 创建一项Cookie
	 */
	private CookieNameHelper createNewCookie(CookieNameEnum cookieName,
			CookieKeyEnum cookieKey, String value) {
		// 首先取得CookieName的配置
		CookieNameConfig cookieNameConfig = CookieNamePolicyParser
				.getCookieNamePolicyMap().get(cookieName);
		CookieNameHelper cookieNameHelper = new CookieNameHelper(
				cookieName.getCookieName(), cookieNameConfig);
		if (cookieNameConfig.isSimpleValue()) {
			cookieNameHelper.updateSimpleValue(value);
		} else {
			cookieNameHelper.update(cookieKey, value);
	...
	public void clear(CookieNameEnum cookieName) {
		CookieNameHelper cookieNameHelper = currentCookieValue.get(cookieName);
		if (cookieNameHelper == null) {
			logger.error("试图更新一个Cookie的值，但是本次请求[" + cookieName
					+ "]不存在！是不是因为域设置有问题。");
			return;
		}
